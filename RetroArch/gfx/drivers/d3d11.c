...
bool my_d3d11_gfx_frame(d3d11_video_t* d3d11, d3d11_texture_t* texture, UINT64 frame_count) {
    ...
    /* ── GBA original size override ──────────────────────────────────────────
   size_data feeds SLANG_SEMANTIC_ORIGINAL uniforms.
   The GPU texture and SourceSize remain at backbuffer res.
   Only the four floats the shader reads for geometry/scanlines are forced
   to GBA native resolution here, after any init that would overwrite them. */
    d3d11->frame.texture[0].size_data.x = 240.0f;
    d3d11->frame.texture[0].size_data.y = 160.0f;
    d3d11->frame.texture[0].size_data.z = 1.0f / 240.0f;
    d3d11->frame.texture[0].size_data.w = 1.0f / 160.0f;
