...
struct PresentShaderState {
    bool                        fired = false;
    bool                        ds = false;
    MyID3D11ShaderResourceView* srv = NULL;
    int                         out_x = 0;
    int                         out_y = 0;
    unsigned                    out_w = 0;
    unsigned                    out_h = 0;
    unsigned                    sc_width = 0;
    unsigned                    sc_height = 0;
};
...
class MyID3D11DeviceContext : public ID3D11DeviceContext {
    ...
    static constexpr uint64_t CTX_MAGIC_ALIVE = 0x434F4E5445585441ull; // "CONTEXTA"
    static constexpr uint64_t CTX_MAGIC_DEAD = 0x434F4E5445585444ull; // "CONTEXTD"

    uint64_t magic = CTX_MAGIC_ALIVE;

public:
    MyID3D11DeviceContext(
    ...
    IUNKNOWN_DECL(ID3D11DeviceContext)

        // Returns the present shader state for swapchain to read at Present time
        PresentShaderState& get_present_shader_state();
    ...
    // Non-virtual
     void sync_cached_size(UINT sc_width, UINT sc_height);
     void sync_render_size(UINT sc_width, UINT sc_height);
     void clear_filter();

     void dump_state(const char* tag, const void* caller) const;
