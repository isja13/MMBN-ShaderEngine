...
class MyID3D11DeviceContext::Impl {
    ...
    void DrawIndexed(
          ...
          //  D3D11_TEXTURE2D_DESC& rtv_tex_desc = rtv_tex->get_desc();
           ...
           /* auto draw_ss = [&](
            ...
            if (filter_next) {
                ...
                if (
                   ...
                   // auto d3d11 = device->get_d3d11_gba();
                    ...
                    if (filter_ss) {
                        ...
                        auto& pss = present_ss;
                        pss.fired = true;
                        pss.ds = filter_state.ds;
                        // NO srv — Present uses bb_copy instead
                        pss.out_x = out_x;
                        pss.out_y = out_y;
                        pss.out_w = out_w;
                        pss.out_h = out_h;
                        pss.sc_width = render_size.sc_width;
                        pss.sc_height = render_size.sc_height;

                        // Let the original draw through — RA composites normally
                        set_psss(device->get_filter_temp().sampler_linear);
                        context->DrawIndexed(IndexCount, StartIndexLocation, BaseVertexLocation);
                        restore_pssss();
                        ...
                        filter = false;
                        filter_next = true;
                        DIAG_LOG("[FIRE_SET] filter_next=%d\n", (int)filter_next);
                    }
                ...
                clear_filter();
                // draw the actual game pixels through before arming
        ...
        DIAG_LOG("[PRE_END] filter_next=%d fired=%d\n", (int)filter_next, (int)present_ss.fired);
    end:
...
PresentShaderState& MyID3D11DeviceContext::get_present_shader_state() {
    return impl->present_ss;
}
