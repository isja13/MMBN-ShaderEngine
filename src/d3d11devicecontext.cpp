...
struct LogItem<MyVertexBufferElement_Logger> : DXGIBufferType {
    const MyVertexBufferElement_Logger& a;  // Change * to &
    LogItem(const MyVertexBufferElement_Logger& a) : a(a) {}  // Change * to &
    void log_item(Logger* l) const {
        l->log_struct_begin();
        auto descs = a.input_layout->get_descs();  // Change -> to .
        auto cached = a.buffer;  // Change -> to .
...
class MyID3D11DeviceContext::Impl {
    ...
    struct FilterState {
        ...
        UINT vertex_stride;
        UINT vertex_offset;
        float key_color[3] = {};   // sampled corner color this frame
    } filter_state = {};
    bool filter = false;
    bool fired_this_frame = false;

   // float key_color[4] = { 0.0f, 0.0f, 0.0f, 0.08f }; // persistent, updated one frame behind
   // bool key_copy_pending = false;
    ...
    void Draw(
        ...
        if constexpr (ENABLE_SLANG_SHADER) {
            filter_ss_gba = device->get_d3d11_gba() && device->get_d3d11_gba()->shader_preset;
            filter_ss_ds = device->get_d3d11_ds() && device->get_d3d11_ds()->shader_preset;
        }
        auto set_filter_state_ps = [this] {
            //LOG("[LOCAL] set_filter_state_ps calling PSSetShader(%p, NULL, 0)",
                //filter_state.ps ? filter_state.ps->get_inner() : nullptr);
            context->PSSetShader(filter_state.ps->get_inner(), NULL, 0);
            };
        auto restore_ps = [this] {
            auto ps = cached_ps ? cached_ps->get_inner() : NULL;
           // LOG("[LOCAL] restore_ps calling PSSetShader(%p, NULL, 0)", ps);
        ...
        auto srv = *cached_pssrvs;
        {
            ...
            auto rtv_tex = (MyID3D11Texture2D*)rtv->get_resource();

            D3D11_TEXTURE2D_DESC& rtv_tex_desc = rtv_tex->get_desc();
            ...
            auto draw_ss = [&](
                ...
                TextureAndViews* cached_tex = NULL
                    ...
                    auto my_texture =
                        as_wrapper<MyID3D11Texture2D>(srv->get_resource());
            ...
            if (filter_next) {
                if (
                    ...
                    auto d3d11 = device->get_d3d11_gba();
                    if (filter_state.ds) {
                        if (filter_ss_ds) {
                            d3d11 = device->get_d3d11_ds();
                            filter_ss = true;
                        }
                    }
                    else {
                        if (filter_ss_gba) {
                            d3d11 = device->get_d3d11_gba();
                            filter_ss = true;
                        }
                    }
                    if (filter_ss) {
        ...
        if (filter_next) goto done;
        {
            ...
            case PS_HASH_T1:
                ...
                set_psss(device->get_filter_temp().sampler_nn);
                context->Draw(
                    VertexCount,
                    StartVertexLocation
                );
            ...
            case PS_HASH_T2: {
                ...
                if (render_interp) {
                    ID3D11SamplerState* sss[2] = {
                        device->get_filter_temp().sampler_wrap,
                        device->get_filter_temp().sampler_linear
                    };
                ...
                if (render_linear) {
                    set_viewport(
                        device->get_filter_temp().tex_t2->width,
                        device->get_filter_temp().tex_t2->height
                    );
                    set_rtv(
                        device->get_filter_temp().tex_t2->rtv,
                        device->get_filter_temp().tex_t2->dsv
                    );
                }
                context->Draw(
                    VertexCount,
                ...
                if (render_linear) {
                    restore_rtvs();
                    restore_vps();
                    set_srv(device->get_filter_temp().tex_t2->srv);
                    set_psss(device->get_filter_temp().sampler_linear);
                    set_filter_state_ps();
                    context->VSSetShader(filter_state.vs->get_inner(), NULL, 0);
                    context->IASetVertexBuffers(
                        0,
                        1,
                        &filter_state.vertex_buffer,
                    ...
                    context->VSSetShader(cached_vs->get_inner(), NULL, 0);
                    restore_ps();
                    restore_pssrvs();
                }
                restore_pssss();
    ...
    void DrawIndexed(
        ...
        auto srv = *cached_pssrvs;
        {
            ...
            // DELTA: IndexCount == 6 instead of VertexCount == 4
            ...
            if (cached_ps && (
                ...
                cached_ps->get_bytecode_hash() == 0xdfeed8d5
                ))
                filter_next = filter;
            else {
                if (!psss) goto end;
          ...
          //  filter_next = filter;
            ...
            // DELTA: DS path retained for future-proofing, costs nothing
            else if (
                srv_tex_desc.Width == DS_WIDTH &&
                srv_tex_desc.Height == DS_HEIGHT
                ) {
          ...
          //  D3D11_TEXTURE2D_DESC& rtv_tex_desc = rtv_tex->get_desc();
            ...
            // DELTA: draw_enhanced and draw_nn use DrawIndexed internally
            auto draw_enhanced = [&](
                std::vector<TextureViewsAndBuffer*>& v_v
                ) {
                    auto v_it = v_v.begin();
                    ...
                    set_it_pscbs();
                    // DELTA: uses start_index_location
            ...
            // DELTA: draw_ss cached_tex final blit uses DrawIndexed
            auto draw_ss = [&](
                d3d11_video_t* d3d11,
                MyID3D11ShaderResourceView* srv,
                D3D11_VIEWPORT* render_vp,
                MyID3D11RenderTargetView* fire_rtv,    // ADD THIS
                TextureAndViews* cached_tex = NULL
                ) {
                    if (cached_tex) {
                        // cached_tex branch unchanged
                        video_viewport_t vp = {
                            .x = 0,
                            .y = 0,
                            .width = cached_tex->width,
                        ...
                        unsigned int out_h = render_size.sc_height;
                        unsigned int out_w = out_h * GBA_WIDTH / GBA_HEIGHT;
                        if (out_w > render_size.sc_width) {
                            out_w = render_size.sc_width;
                            out_h = out_w * GBA_HEIGHT / GBA_WIDTH;
                        }
                        ...
                        // immediately before my_d3d11_update_viewport in the else branch:
                        DIAG_LOG("[DRAW_SS_RTV] fire_rtv=%p inner=%p orig_w=%u orig_h=%u\n",
                            (void*)fire_rtv,
                            (void*)fire_rtv->get_inner(),
                            fire_rtv ? ((MyID3D11Texture2D*)fire_rtv->get_resource())->get_orig_width() : 0,
                            fire_rtv ? ((MyID3D11Texture2D*)fire_rtv->get_resource())->get_orig_height() : 0
                        );
                    ...
                    // DELTA: restore index buffer after slang trashes IA state
                    ...
                    if (cached_tex) {
                        set_psss(device->get_filter_temp().sampler_linear);
                        set_srv(cached_tex->srv);
                        // DELTA: DrawIndexed for the cached_tex final blit
                        context->DrawIndexed(IndexCount, StartIndexLocation, BaseVertexLocation);
                    }
            ...
            if (filter_next) {
                ...
                // DELTA: orig_width/height check same as Draw — guards against
                ...
                if (
                    ...
                    if (filter_ss) {
                       ...
                       draw_ss(
                           ...
                           filter_state.srv,
                           &render_vp,
                           cached_rtv,        // ADD THIS — captured fresh at call time, is the swapchain RTV
                           render_interp ?
                           filter_state.ds ?
                           device->get_filter_temp().tex_nn_ds :
                           device->get_filter_temp().tex_nn_gba :
                ...
                // DELTA: no rtv/srv 2x size relationship to enforce.
                // GBA_WIDTH x GBA_HEIGHT on SRV is already the gate above.
                // T2 hash is the arm condition for this game.
                ...
                if (psss) psss->AddRef();
                filter_state.ds = ds;
                // DELTA: store index location instead of vertex location
        ...
        if (filter_next) goto done;
        {
            // DELTA: T1 and T3 pass through — T1 is the fire pass caught
            ...
            if (cached_ps && cached_ps->get_bytecode_hash() == 0xdfeed8d5) {
            ...
            if (fired_this_frame && cached_ps && cached_ps->get_bytecode_hash() == 0xdfeed8d5) {
...
static inline WrapperT* LookupLiveWrapperAndAddRef(
    ...
    WrapperT* wrapped = nullptr;
    {
        ...
        if (it != cache.end() && it->second && is_wrapper(it->second)) {
            wrapped = it->second;
            wrapped->AddRef(); // pin before leaving lock
        }
    }
    return wrapped;
}
    ...
    for (UINT i = 0; i < numViews; ++i) {
        ...
        MyID3D11ShaderResourceView* wrapped = nullptr;
        {
            ...
            if (it != cached_srvs_map.end() && it->second && is_wrapper(it->second)) {
                wrapped = it->second;
                wrapped->AddRef(); // pin wrapper before releasing inner
            }
        }
...
void STDMETHODCALLTYPE MyID3D11DeviceContext::PSSetSamplers(
    ...
    // 1. Protect the logger! If ppSamplers is NULL, don't use ArrayLoggerDeref.
    if (ppSamplers) {
        LOG_MFUN(_,
            LOG_ARG(StartSlot),
            LOG_ARG(NumSamplers),
        ...
        LOG_MFUN(_,
            LOG_ARG(StartSlot),
            LOG_ARG(NumSamplers),
            LOG_ARG(ppSamplers) // Just log the NULL pointer
        );
    }
    ...
    impl->context->PSSetSamplers(
        StartSlot,
        NumSamplers,
        NumSamplers ? sss : NULL // Always pass our safe array or NULL
    );
}
    ...
    for (UINT i = 0; i < NumBuffers; ++i) {
        ...
        MyID3D11Buffer* wrapped = nullptr;
        {
            ...
            if (it != cached_bs_map.end() && it->second && is_wrapper(it->second)) {
                wrapped = it->second;
                wrapped->AddRef(); // pin wrapper before dropping map lock
            }
        }
...
void STDMETHODCALLTYPE MyID3D11DeviceContext::CSSetSamplers(
    ...
    // 1. We must unwrap before passing to the real context!
