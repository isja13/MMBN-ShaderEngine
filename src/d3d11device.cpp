...
ToggleHotkeyState hk_increase_text_aura;
ToggleHotkeyState hk_decrease_text_aura;
ToggleHotkeyState hk_increase_text_aura_pad;
ToggleHotkeyState hk_decrease_text_aura_pad;
ToggleHotkeyState hk_increase_sharpness;
ToggleHotkeyState hk_decrease_sharpness;
ToggleHotkeyState hk_widescreen;
ToggleHotkeyState hk_widescreen_pad;
ToggleHotkeyState hk_shader_prev;
ToggleHotkeyState hk_shader_next;
ToggleHotkeyState hk_shader_prev_pad;
ToggleHotkeyState hk_shader_next_pad;

ToggleHotkeyState hk_ws_scale_down;
ToggleHotkeyState hk_ws_scale_up;
...
bool PollStepHotkey(const std::vector<BYTE>& combo,
    ToggleHotkeyState& state,
    std::atomic<float>& value,
    float delta) {
    bool pressed = IsHotkeyComboDown(combo);
    if (pressed && !state.was_pressed) {
        value = value.load() + delta;
        state.was_pressed = true;
        return true;
    }
    if (!pressed) state.was_pressed = false;
    return false;
}
// From wikipedia
UINT64 xorshift128p() {
...
#define GET_SET_CONFIG_BOOL(v, m) do { \
    ...
    if (render_ ## v != v) { \
        ...
        GET_SET_CONFIG_BOOL(linear, "Linear filtering");
        GET_SET_CONFIG_BOOL(enhanced, "Enhanced filter");

        if constexpr (ENABLE_SLANG_SHADER) {
            if (config->slang_shader_1_updated ||
                config->slang_shader_2_updated ||
                config->slang_shader_3_updated) {

                config->begin_config();
                bool upd[3] = {
                    config->slang_shader_1_updated,
                    config->slang_shader_2_updated,
                    config->slang_shader_3_updated
                };
                std::string paths[3] = {
                    upd[0] ? config->slang_shader_1 : "",
                    upd[1] ? config->slang_shader_2 : "",
                    upd[2] ? config->slang_shader_3 : ""
                };
                config->slang_shader_1_updated = false;
                config->slang_shader_2_updated = false;
                config->slang_shader_3_updated = false;
                config->end_config();

                for (int i = 0; i < 3; i++) {
                    if (!upd[i]) continue;
                    if (!d3d11_slots[i]) {
                        d3d11_slots[i] = my_d3d11_gfx_init(inner, TEX_FORMAT);
                        if (!d3d11_slots[i]) {
                            overlay("Failed to initialize slang slot ", std::to_string(i + 1));
                            continue;
                        }
                    }
                    if (!paths[i].size()) {
                        my_d3d11_gfx_set_shader(d3d11_slots[i], NULL);
                        overlay("Slang slot ", std::to_string(i + 1), " disabled");
                    }
                    else if (my_d3d11_gfx_set_shader(d3d11_slots[i], paths[i].c_str())) {
                        overlay("Slang slot ", std::to_string(i + 1), " set to ", paths[i]);
                    }
                    else {
                        overlay("Failed to set slang slot ", std::to_string(i + 1), " to ", paths[i]);
                    }
                }
            }
        }
    ...
    void filter_temp_init() {
        ...
        // KEY PS: 1x1 staging texture for composite readback
        {
...

        {
            ...
            bd.BindFlags = D3D11_BIND_CONSTANT_BUFFER;
            bd.CPUAccessFlags = D3D11_CPU_ACCESS_WRITE;
            inner->CreateBuffer(&bd, NULL, &filter_temp.key_vs_cb);
        }
        ...
        // KEY PS: compile inline HLSL
        {
            static const char* key_ps_src =
                ...
                "    float diff = length(c.rgb);\n"
                "    float alpha = saturate((diff - threshold) * sharpness);\n"
        ...
        // KEY VS: widescreen horizontal stretch for text/UI layer
        {
            static const char* key_vs_src =
                "cbuffer Transform : register(b0) { float4x4 mvp; };\n"
                "cbuffer KeyScale  : register(b1) { float ws_scale; float3 _pad; };\n"
            ...
            ID3DBlob* err = nullptr;
            HRESULT hr = D3DCompile(
                key_vs_src, strlen(key_vs_src),
                NULL, NULL, NULL,
                "main", "vs_5_0",
                0, 0, &blob, &err
            );
            if (SUCCEEDED(hr) && blob) {
                inner->CreateVertexShader(
                    blob->GetBufferPointer(),
                    blob->GetBufferSize(),
                    NULL,
                    &filter_temp.key_vs
    ...
    void filter_temp_shutdown() {
        ...
        if (filter_temp.key_vs) filter_temp.key_vs->Release();
        if (filter_temp.key_vs_cb) { filter_temp.key_vs_cb->Release();
        filter_temp.key_vs_cb = nullptr; }
        filter_temp = {};
    }
    ...
    ~Impl() {
        filter_temp_shutdown();
        for (int i = 0; i < 3; i++)
            if (d3d11_slots[i]) my_d3d11_gfx_free(d3d11_slots[i]);

        if (context) context->Release();
    }
};

d3d11_video_t* MyID3D11Device::get_d3d11_slot(int i) const {
    if (i < 0 || i > 2) return nullptr;
    return impl->d3d11_slots[i];
}
...
Config* MyID3D11Device::get_config() const { return impl->config; }
...
HRESULT STDMETHODCALLTYPE MyID3D11Device::QueryInterface(
    ...
    *ppvObject = NULL;
    ...
    // Forward everything else to inner
    HRESULT ret = impl->inner->QueryInterface(riid, ppvObject);
    if (ret == S_OK) {
        LOG_MFUN(_, LOG_ARG(riid), LOG_ARG(*ppvObject), ret);
    }
...
void MyID3D11Device::Impl::PollToggles() {
    ...
    int slang_cur = config->slang_active.load();
    int slang_next = slang_cur;
    bool slang_cycle_changed = false;
    ...
    if (PollStepHotkey(config->hotkey_increase_text_aura,
        hk_increase_text_aura, config->key_threshold, -0.01f))
        threshold_changed = true;
    if (PollStepHotkey(config->hotkey_decrease_text_aura,
        hk_decrease_text_aura, config->key_threshold, 0.01f))
        threshold_changed = true;
    if (PollStepHotkey(config->hotkey_increase_text_aura_pad,
        hk_increase_text_aura_pad, config->key_threshold, -0.01f))
        threshold_changed = true;
    if (PollStepHotkey(config->hotkey_decrease_text_aura_pad,
        hk_decrease_text_aura_pad, config->key_threshold, 0.01f))
        threshold_changed = true;
    
    if (PollStepHotkey(config->hotkey_increase_sharpness,
        hk_increase_sharpness, config->key_sharpness, 0.5f))
        sharpness_changed = true;
    if (PollStepHotkey(config->hotkey_decrease_sharpness,
        hk_decrease_sharpness, config->key_sharpness, -0.5f))
        sharpness_changed = true;

    if (default_overlay && sharpness_changed) {
        char buf[64];
        snprintf(buf, sizeof(buf), "Text Sharpness: %.1f",
            config->key_sharpness.load());
        default_overlay->update_text(buf, "Text Sharpness:");
    }

    if (config->shader_toggle) {
        if (PollToggleHotkey(config->hotkey_widescreen,
            config->hotkey_widescreen_pad,
            hk_widescreen,
            hk_widescreen_pad,
            config->widescreen))
            widescreen_changed = true;
    }

    if (default_overlay && shader_changed) {
        default_overlay->update_text(
            (std::string("Shader: ") + (config->shader_toggle ? "ON" : "OFF")).c_str(),
            "Shader:"
        );
    }
    if (default_overlay && threshold_changed) {
        char buf[64];
        snprintf(buf, sizeof(buf), "Text Aura - threshold : %.2f",
            config->key_threshold.load());
        default_overlay->update_text(buf, "Text Aura - threshold");
    }
    if (default_overlay && widescreen_changed) {
        if (std::abs(context->get_ui_scale() - 1.0f) < 1e-6f) {
            default_overlay->update_text(
                (std::string("Widescreen: ") + (config->widescreen ? "ON" : "OFF")).c_str(),
                "Widescreen:"
            );
        }
        else {
            default_overlay->update_text(
                "Widescreen: unavailable (set window to full size)",
                "Widescreen:"
            );
        }
    }

    bool ws_scale_changed = false;
    if (PollStepHotkey(config->hotkey_increase_ws_scale, hk_ws_scale_up, config->ws_scale, 0.01f)) ws_scale_changed = true;
    if (PollStepHotkey(config->hotkey_decrease_ws_scale, hk_ws_scale_down, config->ws_scale, -0.01f)) ws_scale_changed = true;

    if (default_overlay && ws_scale_changed) {
        char buf[64];
        snprintf(buf, sizeof(buf), "WS Scale: %.4f", config->ws_scale.load());
        default_overlay->update_text(buf, "WS Scale:");
    }

    if (PollToggleHotkey(
        config->hotkey_shader_prev,
        config->hotkey_shader_prev_pad,
        hk_shader_prev,
        hk_shader_prev_pad,
        config->shader_prev_pressed)) {
        // find prev filled slot
        for (int i = 1; i <= 3; i++) {
            int idx = ((slang_cur - i) % 3 + 3) % 3;
            if (d3d11_slots[idx] && d3d11_slots[idx]->shader_preset) {
                slang_next = idx;
                break;
            }
        }
        slang_cycle_changed = slang_next != slang_cur;
    }
    if (PollToggleHotkey(
        config->hotkey_shader_next,
        config->hotkey_shader_next_pad,
        hk_shader_next,
        hk_shader_next_pad,
        config->shader_next_pressed)) {
        for (int i = 1; i <= 3; i++) {
            int idx = (slang_cur + i) % 3;
            if (d3d11_slots[idx] && d3d11_slots[idx]->shader_preset) {
                slang_next = idx;
                break;
            }
    ...
    if (slang_cycle_changed) {
        config->slang_active = slang_next;
        char buf[64];
        snprintf(buf, sizeof(buf), "Slang: preset %d", slang_next + 1);
        if (default_overlay) default_overlay->update_text(buf, "Slang:");
    }

    // Force shader off if in-game filter is not active, update overlay if state changed
    if (config->shader_toggle && context && context->get_gba_seen() && !context->get_ingame_filter_seen()) {
        config->shader_toggle = false;
        if (default_overlay) {
            default_overlay->update_text("Shader: OFF (filter inactive)", "Shader:");
        }
    }
