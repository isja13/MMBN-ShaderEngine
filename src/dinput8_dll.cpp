...
DEFINE_PROC(HRESULT, D3D11CreateDeviceAndSwapChain, (
    ...
    if (SUCCEEDED(ret)) {
        const bool hasSwapChain = (ppSwapChain && *ppSwapChain);
        const bool hasDevice = (ppDevice && *ppDevice);

        // Wrap only the call that actually produced a swap chain + device.
        if (hasSwapChain && hasDevice) {
            DXGI_SWAP_CHAIN_DESC actual_desc = {};
            if (FAILED((*ppSwapChain)->GetDesc(&actual_desc))) {
                ZeroMemory(&actual_desc, sizeof(actual_desc));
                if (pSwapChainDesc) actual_desc = *pSwapChainDesc;
            ...
            if (ppImmediateContext && *ppImmediateContext) {
                ((IUnknown*)(*ppImmediateContext))->Release();
                MyID3D11Device* wrapped_dev = (MyID3D11Device*)(*ppDevice);
                *ppImmediateContext = wrapped_dev->get_context();
                (*ppImmediateContext)->AddRef();
            }
