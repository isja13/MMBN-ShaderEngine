...
class Ini::Impl {
    ...
    bool key_threshold_loaded = false;
...
#undef UPDATED_VAR
        ...
        if constexpr (ENABLE_SLANG_SHADER) {
            GET_SET_CONFIG_UTF8_VALUE(slang_shader_1);
            GET_SET_CONFIG_UTF8_VALUE(slang_shader_2);
            GET_SET_CONFIG_UTF8_VALUE(slang_shader_3);
        }
...
{
    GET_INI_VALUE(hotkey_increase_text_aura);
    config->hotkey_increase_text_aura = ini_parse_vk_comb(returned_string);
    if (!config->hotkey_increase_text_aura.size()) {
        config->hotkey_increase_text_aura = { VK_OEM_PLUS };
    }
}
{
    GET_INI_VALUE(hotkey_decrease_text_aura);
    config->hotkey_decrease_text_aura = ini_parse_vk_comb(returned_string);
    if (!config->hotkey_decrease_text_aura.size()) {
        config->hotkey_decrease_text_aura   = { VK_OEM_MINUS};
    }
    {
        GET_INI_VALUE(hotkey_widescreen);
        config->hotkey_widescreen = ini_parse_vk_comb(returned_string);
    }
    {
        GET_INI_VALUE(hotkey_increase_ws_scale);
        config->hotkey_increase_ws_scale = ini_parse_vk_comb(returned_string);
    }
    {
        GET_INI_VALUE(hotkey_decrease_ws_scale);
        config->hotkey_decrease_ws_scale = ini_parse_vk_comb(returned_string);
    }
    {
        GET_INI_VALUE(hotkey_shader_prev);
        config->hotkey_shader_prev = ini_parse_vk_comb(returned_string);
        if (!config->hotkey_shader_prev.size()) {
            config->hotkey_shader_prev = { '1' };
        }
    }
    {
        GET_INI_VALUE(hotkey_shader_next);
        config->hotkey_shader_next = ini_parse_vk_comb(returned_string);
        if (!config->hotkey_shader_next.size()) {
            config->hotkey_shader_next = { '2' };
        }
    }
    {
        GET_INI_VALUE(hotkey_increase_sharpness);
        config->hotkey_increase_sharpness = ini_parse_vk_comb(returned_string);
    }
    {
        GET_INI_VALUE(hotkey_decrease_sharpness);
        config->hotkey_decrease_sharpness = ini_parse_vk_comb(returned_string);
    }
}
#undef SECTION

#define SECTION controller
{
    {
...
{
    GET_INI_VALUE(hotkey_increase_text_aura_pad);
    config->hotkey_increase_text_aura_pad = ini_parse_vk_comb(returned_string);
}
{
    GET_INI_VALUE(hotkey_decrease_text_aura_pad);
    config->hotkey_decrease_text_aura_pad = ini_parse_vk_comb(returned_string);
}
{
    GET_INI_VALUE(hotkey_widescreen_pad);
    config->hotkey_widescreen_pad = ini_parse_vk_comb(returned_string);
}
{
    GET_INI_VALUE(hotkey_shader_prev_pad);
    config->hotkey_shader_prev_pad = ini_parse_vk_comb(returned_string);
}
{
    GET_INI_VALUE(hotkey_shader_next_pad);
    config->hotkey_shader_next_pad = ini_parse_vk_comb(returned_string);
    if (!config->hotkey_shader_next_pad.size()) {
        config->hotkey_shader_next_pad = { XINPUT_VK_RS };
    }
}
#undef SECTION
...
{
    if (!key_threshold_loaded) {
        GET_INI_VALUE(key_threshold);
        if (*returned_string) {
            TCHAR* end;
            float val = (float)_tcstod(returned_string, &end);
            if (end == returned_string || *end != _T('\0')) {
                OVERLAY_PUSH_INVALID_VALUE(key_threshold);
            }
            else {
                config->key_threshold = val;
                key_threshold_loaded = true;
            }
        }
    }
    {
        GET_INI_VALUE(widescreen);
        if (*returned_string) {
            bool val = _tcsicmp(returned_string, _T("TRUE")) == 0;
            if (!val && _tcsicmp(returned_string, _T("FALSE")) != 0) {
                OVERLAY_PUSH_INVALID_VALUE(widescreen);
            }
            else {
                config->widescreen = val;
            }
        }
    }
    {
        GET_INI_VALUE(ws_scale);
        if (*returned_string) {
            TCHAR* end;
            float val = (float)_tcstod(returned_string, &end);
            if (end == returned_string || *end != _T('\0')) {
                OVERLAY_PUSH_INVALID_VALUE(ws_scale);
            }
            else {
                config->ws_scale = val;
            }
    ...
    {
        GET_INI_VALUE(key_sharpness);
        if (*returned_string) {
            TCHAR* end;
            float val = (float)_tcstod(returned_string, &end);
            if (end == returned_string || *end != _T('\0')) {
                OVERLAY_PUSH_INVALID_VALUE(key_sharpness);
            }
            else {
                config->key_sharpness = val;
            }
        }
    }
}
