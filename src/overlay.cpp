...
#define TEXT_DURATION 10.0  // change if you think it's overlong
...
class Overlay::Impl {
    ...
    void update_text_base(std::string&& s, const std::string& prefix) {
        for (auto& text : texts) {
            if (text.text.substr(0, prefix.size()) == prefix) {
                text.text = std::move(s);
                text.time = 0; // reset duration so it stays visible
                return;
            }
        }
        push_text_base(std::move(s));
    }
...
void Overlay::update_text_base(std::string&& s, const std::string& prefix) {
    impl->begin_text();
    impl->update_text_base(std::move(s), prefix);
    impl->end_text();
}
